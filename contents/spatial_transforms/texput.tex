...
subsystems to minimize the $J$ tradeoff between $D$ and $R$ (in the
KLT we only minimize $D$). $\lambda$ controls our preference:
\begin{enumerate}
  \item If $\lambda$ is small, for a given quantization step size and
    entropy encoding parameters, we are interested in finding a transformation matrix a high quality
    reconstruction.
