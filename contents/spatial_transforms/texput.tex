...
\item Using RDO, determine the optimal DWT basis (defined in
  ...
  the codec \texttt{2D-DWT.py}. Complexity 4.
\item Create a new image codec (similar to \texttt{2D-DCT.py}) where
  ...
  block, use the second block to retrain the model, and so
  on. Complexity 16.
\item Create a new image codec (similar to \texttt{2D-DWT.py}) where
  ...
  analyzed spatial resolution levels to fine-tune the
  coefficients. Complexity 16.
