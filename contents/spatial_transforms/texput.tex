...
\item \textbf{Learned Block Transform (LBT)}: As other transforms do,
  ...
  concentration. Implement an non-compressive
  \href{https://en.wikipedia.org/wiki/Autoencoder}{autoencoder}
  trained (using supervised learning) to maximize the
  ...
  image to compress. The size (and shape) of the layer (included the
  input) should the same, but configurable (typically between 4x4, 8x8
  and 16x16). Notice that, during inference, the output of the neurons
  located in the central layer should represents the coefficients of
  the learned transform, which are the values that are going to be
  quantized and compressed. Remember that we will need also the
  inverse transform.
\end{enumerate}
