...
are capable of matching correspondences between input and
output tensors. If we design a 2-layers autoencoder where the neurons
do not use any activation function and we train it to minimize
\begin{equation}
  J = D + \lambda R,
\end{equation}
...
\section{To-Do}
The current implementation of LBT is not separable (the transform
matrices has been found for the complete blocks). If $T(\mathbf{X})$
represents the transform of the block $\mathbf{X}$ modify
\href{https://github.com/Sistemas-Multimedia/VCF/blob/main/notebooks/2D-LBT.py}{2D-LBT}
to train $\mathbf{A}\in\mathbb{R}^{B\times B}$, where $B$ is the side
of the square blocks, being\footnote{In the current implementation,
\begin{equation}
...
Sumarizing, the only that should be modified is that the transform is
applied first by rows and the by columns.
