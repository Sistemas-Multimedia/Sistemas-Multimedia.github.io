...
DCT~\cite{vruiz__DCT}, each coefficient represents a different
...
DWT~\cite{vruiz__DWT}, the coefficients ``speak'' additionally
about a spatial resolution and position.
...
\emph{blocks}), and apply the encoding to each chunk,
...
\item In general, blocks are smaller than tiles and, in most of cases,
...
\item
  Autoencoders can be used in the transform domain. Modify VCF to use
  an autoencoder with a small squared input to compress the 2D-DWT
