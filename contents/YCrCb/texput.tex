...
\input{../../definitions}
\title{\SM{} - Study Guide - Image Compression using the $\text{YCrCb}$ Color Transform}

\maketitle
\tableofcontents

\section{Objective}
Let's use the $\text{YCrCb}$ color (from the \text{RGB} domain)
transform to compress $\text{RGB}$ images. The $\text{YCrCb}$ is a
\emph{luma}-based (luminance-based\footnote{Luminance can be
considered as the intensity part of a viual stimuli.} color domain.
