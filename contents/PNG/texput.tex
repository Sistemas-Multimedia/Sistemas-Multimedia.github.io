...
\title{\SM{} - Study Guide - Milestone 4: Entropy coding with
...
\href{https://vicente-gonzalez-ruiz.github.io/PNG/}{PNG}, the
...
(or \href{https://en.wikipedia.org/wiki/Film_frame}{frames}, in the
context of video).
...
\href{https://en.wikipedia.org/wiki/Charge-coupled_device}{CCD (Charge
  Coupled Device)}. In maths, an image (or a frame) is
\begin{equation}
  {\mathbf I} = \{{\mathbf I}_{y,x}\}\in\mathbb{P}^{k}, (y,x)\in\mathbb{N}^2\}
    \label{eq:image}
\end{equation}
where $(y,x)$ is a spatial coordinate, and $\mathbb{P}^{k}$ represents
the (pixels) \emph{values domain}, with dimension (number of pixel
components) $k$~\cite{burger2016digital}.

In a similar way, a sequence frames can be denoted by
\begin{equation}
...
as the described in Eq.~\ref{eq:image}.
...
\item Run this \href{https://jupyter.org/}{jupyter}
  \href{https://github.com/Sistemas-Multimedia/Sistemas-Multimedia.github.io/blob/master/milestones/PNG/display_video.ipynb}{notebook}
