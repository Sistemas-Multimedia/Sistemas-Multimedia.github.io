...
\item Create a new module named \texttt{adaptive\_Huffman.py} that use
  ...
  select the order of the model, and HC is used as an entropy encoder.
\end{enumerate}
...
\item Create a new module named \texttt{arith} that implement a static
  ...
  \cite{nelson96datacompression} to be used in VCF. Complexity 5.
\item Create a new module named \texttt{arith\_a.py} that use
