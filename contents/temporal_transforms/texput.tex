...
reversible process.} at the decoder. The better the prediction, the
lower the entropy of the residuals. Notice that the number of
residual-pixels is equal to the number of pixels in the compensated
frame.

Temporal transforms based on MC are called motion compensated
transforms~\cite{vruiz__MCT}, which generally increase the dynamic
range of the signal.
...
mapping of the pixels from one frame (2D image) to
another~\cite{vruiz__ME}. Notice that the mapping pattern must be
available at both, the encoder and the decoder side, in order to make
this a reversible process.

% Up to here, sistemas multimedia's contents
