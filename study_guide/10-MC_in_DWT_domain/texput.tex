...
is consequence of that the filters used for creating the LP levels are
not orthogonal and therefore, they do not cancel the aliasing between
them (see this
\href{https://github.com/Sistemas-Multimedia/Sistemas-Multimedia.github.io/blob/master/study_guide/10-MC_in_DWT_domain/LP_is_not_critical.ipynb}{notebook}). Moreover,
the redundancy in the LP tends to 2 with the number of levels, which
affects negatively to the compression ratio. An adventage of this is
that the LP is shift-invariant in the high-frequency subband (see the
...
\begin{figure}
  \centering
  \begin{tabular}{ccc}
    \vbox{\png{f0_CS-LPT_level1}{300}} & \vbox{\png{f1_CS-LPT_level1}{300}} & \vbox{\png{f2_CS-LPT_level1}{300}} \\
    \vbox{\png{f0_CS-LPT_level0}{300}} & \vbox{\png{f1_CS-LPT_level0}{300}} & \vbox{\png{f2_CS-LPT_level0}{300}} \\
    & \vbox{\svg{f0_1_CS-LPT_level1}{300}} & \vbox{\svg{f0_2_CS-LPT_level1}{300}} \\
    & \vbox{\svg{f0_1_CS-LPT_level0}{300}} & \vbox{\svg{f0_2_CS-LPT_level0}{300}}
  \end{tabular}
  \caption{A demonstration of the shift-invariance of the CS-LPT. See
    this
    \href{https://github.com/Sistemas-Multimedia/Sistemas-Multimedia.github.io/blob/master/study_guide/10-MC_in_DWT_domain/CS-LPT_shift_invariance.ipynb}{notebook}.}
\label{fig:CS-LP}
\end{figure}

CS-LPT is a special case of the LP where the filters are
(bi)orthogonal DWT filters (in this case, we say that the LP is a
tight frame). The 1-levels CS-LPT (that has two levels in its pyramid)
...
The CS-LPT is only near shift-invariant as it can be seen in this
\href{https://github.com/Sistemas-Multimedia/Sistemas-Multimedia.github.io/blob/master/study_guide/10-MC_in_DWT_domain/shift_invariance_in_CS-LPT.ipynb}{notebook}. However,
it has several advantages:
\begin{enumerate}
\item The CS-LPT more compact than LP and ODWT.
...
\item Interpolate the low-frequency subbands of $R$ and $P$:
  \begin{equation}
    [P.L] = \text{DWT}^{-1}(P.L, 0),
  \end{equation}
  \begin{equation}
...
\item Output
  \begin{equation}
    O_{i,j} = \left\{
      \begin{array}{ll}
        [P.L]_{i,j} & \text{if} M_{i,j} = 0 (\text{intra~pixel})\\
        {[}E.L{]}_{i,j} & \text{otherwise} (\text{predicted-pixel}).
      \end{array}
    \right.
    \label{eq:output}
  \end{equation}
  ...
  $O$ is the high-frequency subband a that we will send from the
