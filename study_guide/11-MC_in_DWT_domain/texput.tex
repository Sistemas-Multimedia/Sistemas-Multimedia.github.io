...
\begin{figure}
  \centering
  \begin{tabular}{ccc}
    \vbox{\png{f0_haar_H}{300}} & \vbox{\png{f1_haar_H}{300}} & \vbox{\png{f2_haar_H}{300}} \\
    & \vbox{\svg{f0_1_haar_H}{300}} & \vbox{\svg{f0_2_haar_H}{300}}
  \end{tabular}
  \caption{A demonstration of the near shift-invariance in the [H]
    subband of the CS-LPT. See this
...
The CS-LPT is only near shift-invariant as it can be seen in the
Fig.~\ref{fig:CS-LPT}. However, it has several advantages:
\begin{enumerate}
...
\item The error generated by the lack of shift-invariance for the
  odd-type motion is smaller than for the DWT.
\end{enumerate}
...
\item Output
  \begin{equation}
    O_{i,j} = \left\{
      \begin{array}{ll}
        [P.L]_{i,j} & \text{if} M_{i,j} = 0~(\text{intra~coefficient})\\
        {[}E.L{]}_{i,j} & \text{otherwise}~(\text{predicted~coefficient}).
      \end{array}
    \right.
    \label{eq:output}
  \end{equation}
