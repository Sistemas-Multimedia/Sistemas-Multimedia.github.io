...
analysis matrix transform $K$ (see the previous milestone). Haar
...
\item There exists
  ...
  because: \begin{enumerate} \item The same
    ...
    Eq.~\ref{eq:Haar_transform}, the subbands are downsampled by 2 and
    both subbands should have, at most, a
    ...
    of the low-pass subband $l$.  \end{enumerate} These problems can be
  only solved using filters that have
...
\begin{itemize}
\item The resolution level $S$ of $l^0$ (no $\text{DWT}^{-1}$ has been
  applied) $l^S$, with have only one sample $l^S_0$ that uses to be the
