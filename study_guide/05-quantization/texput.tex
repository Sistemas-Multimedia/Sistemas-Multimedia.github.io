...
A color RGB image is quantized chanel by channel, using QSs
$\Delta_{\text{R}}$, $\Delta_{\text{G}}$, and $\Delta_{\text{B}}$. A
reasonable question here is, how we should be chosen these parameters?
...
In the second one, only the RD performance is considered. From a RD
perspective, the best combination of QSs is those that minimizes the
RD curve. Thus, the optimal QSs should operate in the curves with the
same RD slope, which implies that the contribution of each channel
(the ratio between quality and bit-rate) to the quality of $\tilde{x}$
has been best
...
similar, we can suppose that the length (the bit-rate) of each channel
is proportional to the variance of the channel, which is inversely
proportional to the QS, and therefore, Eq.~\ref{eq:simple_Q} holds.
