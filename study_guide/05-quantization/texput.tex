...
Normal RD curves are convex, which means that if $\lambda_i$ is the
slope of the curve measured at the $i$-th point of the curve (starting
at the lowest bit-rate), it always hold that
\begin{equation}
...
\end{equation}
then the optimal QSs must satisfy that~\cite{vetterli1995wavelets,sayood2017introduction}
\begin{equation}
...
so that
\begin{equation*}
  \lambda_{\text{R}} = 2\lambda_{\text{G}} = 2\lambda_{\text{B}},
\end{equation*}
and that we still have room for spending more bits encoding the
frame. In this situation, the maximum benefit would be obtained if we
decrease $\Delta_{\text{R}}$ because the sloper is double the slope of
the other curves. Therefore, the optimal QSs are obtained when
