...
transform because they form an orthonormal basis set, and the elements
of the transformed sequence are often called the transform
coefficients. Obviously, the inverse transform is computed by
\begin{equation}
...
transpose o ${\mathbf A}$.

The DCT is usually applied by blocks which, in most of the previous
image and video compression standards, are of 8x8 pixels. This a
direct consequence of that, usually (depending on the signal
characteristics) the DCT loss compactation efficiency when the block
size is increased. When we group the coefficients of several blocks
are considered together, they form a subband, and the collection of
subbands, a decomposition~\cite{Kovacevic...}.
