\input{../../definitions}
\title{\SM{} - Study Guide - Milestone 6: Compressing with a color transform}
...
will compare are:
\begin{enumerate}
  \item \href{https://en.wikipedia.org/wiki/RGB_color_model}{$\text{RGB}$}
    (the original one),
  \item the
    \href{https://en.wikipedia.org/wiki/Discrete_cosine_transform}{DCT
      (Discrete Cosine Transform)} applied to the color domain, i.e.,
    the color-DCT space,
  \item the \href{https://en.wikipedia.org/wiki/YCbCr}{$\text{YCrCb}$}, and
  \item the \href{https://en.wikipedia.org/wiki/YCoCg}{$\text{YCoCg}$}.
\end{enumerate}
The last two, which are \emph{luma}-based (luminance-based) color
...
\item Since the \href{https://en.wikipedia.org/wiki/Visual_system}{HVS
  ...
  signal, the amount of information, and consequently
  ...
  distortion (see Fig.~\ref{fig:san-diego_chroma_subsampled}). This
  \href{https://en.wikipedia.org/wiki/Bandwidth_(computing)}{fact is
    also used when compressing} digital still images and is why, for
  example, the \href{https://en.wikipedia.org/wiki/JPEG}{JPEG} codec
  converts RGB images to $\text{YCrCb}$. See also this
...
transform coefficients to their geometric mean. Notice that $G$ is
computed inside of a block, not among blocks.
...
The synthesis filters of orthonormal transforms have exactly the unity
gain.\footnote{The quantization error is the same in all the subbands
because all of them have exactly the same gain.} Therefore, without
considering that the entropy coding stage can performs better for some
subbands, the optimal quantization pattern should be
\begin{equation}
  \Delta_{\text{DCT0}} = \Delta_{\text{DCT1}} = \Delta_{\text{DCT2}}.
\end{equation}
See this \href{https://github.com/Sistemas-Multimedia/Sistemas-Multimedia.github.io/blob/master/study_guide/06-color_transform/color-DCT_quantization.ipynb.ipynb}{notebook}.

Notice that to find the gains (of any 1D transform) we can compute the energy of the signal
generated by the inverse transform of the unitary impulse discrete 1D
signal
\begin{equation}
  \delta_{i}(x) = 
  \left\{
...
\href{https://en.wikipedia.org/wiki/YCbCr}{$\text{YCrCb}$} one, we use the $\text{RGB/YCrCb}$ (analysis)
...
components. In such case, there is not optimal
\emph{constant}\footnote{Always using the same relative quantization
steps for each RD point.} quantization pattern and the only way of
finding a good quantization of the components consists in computing
the RD slopes (measured in the RGB domain) of each quantization point,
sort the points by their slope, and to apply the corresponding
quantization patterns (see this
\href{https://github.com/Sistemas-Multimedia/Sistemas-Multimedia.github.io/blob/master/study_guide/06-color_transform/YCrCb_quantization.ipynb.ipynb}{notebook}).
...
entropy encoding stage (that could compress more some components), the
previous channel gains suggest to use
\begin{equation}
