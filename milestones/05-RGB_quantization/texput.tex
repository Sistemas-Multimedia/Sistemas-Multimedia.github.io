...
\href{https://en.wikipedia.org/wiki/RGB_color_model}{RGB\footnote{Red,
  Green and Blue: the primary colors.} color domain}. RGB is an additive color system, which means that all colors start with black and are created by adding the primary colors~\cite{urger2016digital}}. A
...
are uniformly distributed within the RGB cube. However, the typical
color distribution in natural images is anything but uniform, with
some regions of the color space being densely populated and many
colors entirely missing. In this case, scalar quantization is not
optimal because the interesting colors may not be sampled with
suﬃcient density while at the same time colors are represented that do
not appear in the image at all~\cite{burger2016digital}.
...
\begin{enumerate}
\item to
ﬁnd a set of $n$ representative color vectors $C' = \{c'_1, c'_2
,\ldots , c'_n \}$ and
\item to replace each original color $C_i$ by one of
the new color vectors $C'_j\in C'$, where $n$ is usually predetermined
($n < m$) and the resulting deviation from the original image shall be
